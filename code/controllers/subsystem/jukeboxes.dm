...
#define JUKE_SOUND 5
#define JUKE_DISTANCE_MULTIPLIER 2.5
		...
		for(var/mob/M in GLOB.player_list)
			...
			if(source_pressure)
				...
				if(pressure_factor && targetfalloff && M.can_hear() && (hearerturf.z in audible_zlevels))
					...
					song_played.x = (currentturf.x - hearerturf.x) * JUKE_DISTANCE_MULTIPLIER
					song_played.z = (currentturf.y - hearerturf.y) * JUKE_DISTANCE_MULTIPLIER
					song_played.y = (((currentturf.z - hearerturf.z) * 10 * JUKE_DISTANCE_MULTIPLIER) + ((currentturf.z < hearerturf.z) ? -5 : 5))
