...
//and changing allowed_firings to like.... 12? - That's our standard now.
...
/datum/controller/subsystem/monster_wave/fire(resumed = 0)
	...
	for(var/M in GLOB.player_list)
		to_chat(M, "<span class='notice'>You feel the ground tremor subtly beneath your feet.</font></span>")

/datum/controller/subsystem/monster_wave/proc/spawn_monsterwave()

	//--- TRIGGER WORLD TREMORS ---
	for(var/X in GLOB.landmarks_list)
		if(istype(X, /obj/effect/landmark/tremor_area))
			var/obj/effect/landmark/tremor_area/TA = X
			TA.tremor()
