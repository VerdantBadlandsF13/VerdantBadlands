/obj/machinery/jukebox
	...
	verb_say = "states"
	density = TRUE
	req_one_access = null//list(ACCESS_BAR, ACCESS_KITCHEN, ACCESS_HYDROPONICS, ACCESS_ENGINE, ACCESS_CARGO, ACCESS_THEATRE)
	var/active = FALSE
...
/obj/machinery/jukebox/ui_status(mob/user)
	...
	if(!SSjukeboxes.songs.len && !isobserver(user))
		to_chat(user,"<span class='warning'>Error: No music tracks have been found.</span>")
		...
		if("add_to_queue")
			...
			if(!allowed(usr))
				if(isliving(usr))
					...
					playsound(src, 'sound/misc/compiler-failure.ogg', 25, TRUE)
					return
				to_chat(usr, "<span class='notice'>You queue [selectedtrack.song_name].</span>")
