...
/mob/living/carbon/proc/natural_bodytemperature_stabilization()
	if(HAS_TRAIT(src, TRAIT_COLDBLOODED) || isrobotic(src))
...
/mob/living/carbon/proc/get_cooling_efficiency()
	...
	var/coolant_efficiency = min(blood_volume / BLOOD_VOLUME_SAFE, 1) //Low coolant is only a negative, adding more than needed will not help you.
...
/mob/living/carbon/proc/check_suitlinking()
	...
	//var/turf/T = get_turf(src) //Used exclusively in the lavaland check, which is also commented out

	//I mean, I guess if someone finds, and, wants to walk around in a space suit in Fallout... Power to em?
