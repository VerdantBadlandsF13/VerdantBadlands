...
/datum/nanite_program/comm/hallucination/set_extra_setting(setting, value)
	...
	if(setting == NES_HALLUCINATION_TYPE)
		switch(value)
			...
			if("Alert")
				extra_settings[NES_HALLUCINATION_DETAIL] = new /datum/nanite_extra_setting/type("random", list("random","not_enough_tox","not_enough_co2","too_much_co2","too_much_tox","newlaw","nutrition","charge","gravity","fire","locked","hacked","temphot","tempcold","pressure"))
			else
