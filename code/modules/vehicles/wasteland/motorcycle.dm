...
/obj/vehicle/ridden/fuel/motorcycle/relaymove(mob/user)
	if(ishuman(user))
		...
		if(istype(H.wear_suit, /obj/item/clothing/suit/armored/f13/power_armor))//Standard PA.
			to_chat(user, "The [name] will not move, because you are too heavy.")
			return
		if(istype(H.wear_suit, /obj/item/clothing/suit/armored/f13/heavy))//Salvaged PA and misc.
			to_chat(user, "The [name] will not move, because you are too heavy.")
			return
		if(H.pulling)
