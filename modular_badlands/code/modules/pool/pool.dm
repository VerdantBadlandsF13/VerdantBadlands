...
- - - - - - - - - - - - - - - -
All credit to the original port from NSV, to the eventual end destination of Yog(?). Thanks, lads. (PowerfulBacon, KMC, Cdey, Maxim)
For our purposes, it's effectively just for rivers / standing water. Potentially basins in the future. - Carl
- - - - - - - - - - - - - - - -
*/
...
/obj/structure/pool_ladder
	name = "Pool ladder"
	desc = "A sturdy ladder. What did you expect?"
	icon = 'modular_badlands/code/modules/pool/icons/pool.dmi'
	icon_state = "ladder"
	pixel_y = 12
	var/reversed = FALSE
...
/obj/machinery/pool_filter
	name = "water filter"
	desc = "A device which can help you regulate conditions in nearby bodies of water. <br>\
	Use a <b>wrench</b> to change its operating temperature, or hit it with a reagent container to load in new liquids for dispersal."
	icon = 'modular_badlands/code/modules/pool/icons/pool.dmi'
...
/obj/structure/pool_ladder/attack_hand(mob/user)
	...
	if(S)
		to_chat(user, "<span class='notice'>You start to climb out of \the [src]...</span>")
		...
		to_chat(user, "<span class='notice'>You start to climb into \the [src]...</span>")
